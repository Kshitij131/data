...
class GeminiAgent:
    def __init__(self):
        ...
        # Use gemini-1.5-flash based on testing of available models
    ...
    def _post_process_validation_errors(self, errors):
        ...
        This function examines the validation errors from Gemini and filters out:
    ...
    def map_header(self, bad_header: str) -> str:
        ...
        Tries Gemini first; falls back to FAISS if needed.
        """
        prompt = f"""
        ...
        try:
            response = self.model.generate_content(prompt).text.strip().replace('"', '')
    ...
    def validate_data(self, row_data: Union[str, Dict, List]) -> dict:
        ...
        try:
            ...
            gemini_errors = self._safe_json_response(response) or []
            
            # If we couldn't parse any errors but there is response text, create a single error
            if not gemini_errors and response.strip():
                gemini_errors = [{
            ...
            for error in gemini_errors + [vars(e) for e in faiss_errors]:
    ...
    def query_to_filter(self, nl_query: str) -> dict:
        ...
        "{nl_query}"
        """
        response = self._safe_json_response(self.model.generate_content(prompt).text)
        ...
        if response and "filter" in response:
            try:
                ...
                # Ask Gemini to verify and refine the filter
                ...
                improved_response = self._safe_json_response(
                    self.model.generate_content(verification_prompt).text
                )
    ...
    def modify_data(self, nl_command: str) -> dict:
        ...
        if response:
            try:
                ...
                # Ask Gemini to validate and ensure data consistency
                validation_prompt = f"""
                ...
                validated_response = self._safe_json_response(
                    self.model.generate_content(validation_prompt).text
                )
    ...
    def suggest_correction(self, error_row: str) -> dict:
        """
        Use FAISS and Gemini to suggest intelligent corrections for invalid data.
        ...
        if response and "corrected_data" in response:
            ...
            final_response = self._safe_json_response(
                self.model.generate_content(retry_prompt).text
            )
            if final_response:
    ...
    def nl_to_rule(self, rule_text: str) -> dict:
        ...
        if response:
            try:
                ...
                # Ask Gemini to validate rule consistency
                validation_prompt = f"""
    ...
    def rule_recommendations(self, context: str) -> dict:
        ...
        if response and "recommendations" in response:
            try:
                ...
                # Ask Gemini to refine recommendations based on validation
                refinement_prompt = f"""
                ...
                refined_response = self._safe_json_response(
                    self.model.generate_content(refinement_prompt).text
                )
                if refined_response:
    ...
    def generate_priority_profile(self, summary: str) -> dict:
        ...
        if response and "priority_weights" in response:
            try:
                ...
                # Ask Gemini to optimize the profile
                ...
                optimized_response = self._safe_json_response(
                    self.model.generate_content(optimization_prompt).text
                )
